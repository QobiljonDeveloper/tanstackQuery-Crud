...
const Cards = ({
  ...
  setSelectedPhone: (p: Phone) => void;
}) => {
  const { getPhones, deletePhone } = usePhones();
        ...
        {phones.map((phone) => (
          <Card
            ...
            cover={
              <img
                alt={phone.title}
                src={phone.image || defaultImage}
                style={{ height: 200, objectFit: "cover" }}
              />
            }
            actions={[
              <Button type="link" onClick={() => setSelectedPhone(phone)}>
                Update
              </Button>,
              <Popconfirm
          ...
          >
            <Meta
              title={
                <span className="font-semibold text-lg">{phone.title}</span>
              }
              description={
                <div className="mt-2 space-y-1 text-sm text-gray-700">
                  ...
                  <p>
                    Memories:{" "}
                    {Array.isArray(phone.memories)
                      ? phone.memories.join(", ")
                      : phone.memories || "-"}
                  </p>
                  <p>Delivery: {phone.isDelivery ? "Yes" : "No"}</p>
                </div>
              }
            />
          </Card>
